...
const filterPram = filterForm.elements["german"];
...
function renderStudents(students) {
  ...
  students.forEach((student) => {
    ...
    studentCard.innerHTML = `<span>${student.email}</span>, <span>${student.german}</span>`; // опасно. могут передать вредоносный скрипт
...
// удаляет всех из списка и обновляет отображение
function removeAll() {
  students = [];
  renderStudents(students);
}
...
function filterStudents(event) {
  event.preventDefault();
  const filtered = students.filter((student) => student.german === filterPram.value);
